...
echo -e "${YELLOW}=== [Cross-Chain Oracle] 极客全境终极点火序列 v5.0 ===${NC}"
...
# 2. 唤醒 FISCO BCOS
echo -e "${YELLOW}[2/6] 正在唤醒 FISCO BCOS 底层节点...${NC}"
...
# 3. 唤醒 Fabric 与智能合约
echo -e "${YELLOW}[3/6] 正在处理 Hyperledger Fabric 网络...${NC}"
...
echo -e "${YELLOW}[4/6] 正在启动 Chainlink 预言机舰队...${NC}"
...
# 5. 预编译跨链核心微服务 (新增源头存证系统)
echo -e "${YELLOW}[5/6] 正在预编译跨链五大核心微服务...${NC}"
cd "$PROJECT_DIR"
...
# 6. 点火全套微服务后台
echo -e "${YELLOW}[6/6] 正在后台启动全套跨链微服务...${NC}"
cd "$PROJECT_DIR"
